...
function restoreScroll(y){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    if(Math.abs(stableScrollY()-y)>1)window.scrollTo({top:y,left:0,behavior:'auto'});
  }));
}
...
function renderPicker(){
  ...
  if($('#roulette').classList.contains('active'))restoreScroll(keepY);
}
