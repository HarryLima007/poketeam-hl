...
const TRANSFORMABLE=new Set([
  25,351,382,383,386,413,421,422,423,479,483,484,487,492,493,550,585,586,641,642,645,646,647,648,658,666,669,670,671,676,678,681,710,711,718,720,735,738,741,743,744,745,746,752,754,758,774,777,778,784,800,801,845,849,875,876,877,888,889,890,892,893,898,901,902,905,916,925,931,964,978,982,999,1007,1008,1017,1024
]);
...
function preserveViewportDuring(renderFn){
  ...
  const minH=Math.max(root.scrollHeight,body.scrollHeight,window.innerHeight+y+2);
  const prevMin=body.style.minHeight,prevAnchor=root.style.overflowAnchor;
...
async function ensureTransformationArt(list){
  if(!list.length)return;
  await Promise.allSettled(list.map(p=>getTransformationForms(p.id)));
}
...
function paradoxSections(list){
  const groups=[
    ['Paradoxos do Passado',list.filter(p=>PARADOX_PAST.has(p.id))],
    ['Paradoxos do Futuro',list.filter(p=>PARADOX_FUTURE.has(p.id))]
  ];
  return groups.map(([title,arr])=>{
    if(!arr.length)return '';
    const cards=arr.map(p=>cardHTML(p,state.dexShiny));
...
function cardHTML(p,shiny=false,special=null){
  ...
  const name=special?.form?(special.form.label||specialFormLabel(special.form.formName,baseName)):cap(baseName);
...
function generationOfId(id){
  if(id<=151)return 1;if(id<=251)return 2;if(id<=386)return 3;if(id<=493)return 4;if(id<=649)return 5;
  if(id<=721)return 6;if(id<=809)return 7;if(id<=905)return 8;return 9;
}
function formMechanismInfo(form){
  const group=transformationGroup(form);
  const info={
    alola:['Forma Regional','Variante regional de Alola'],
    galar:['Forma Regional','Variante regional de Galar'],
    hisui:['Forma Regional','Variante regional de Hisui'],
    paldea:['Forma Regional','Variante regional de Paldea'],
    fusion:['Fusão','Combinação de dois Pokémon em uma forma única'],
    power:['Transformação de Poder','Mudança de forma associada a aumento de poder'],
    item:['Mudança por Item','Forma obtida por item, objeto-chave ou condição especial'],
    battle:['Mudança em Batalha','Forma que muda durante ou por efeito da batalha'],
    environment:['Clima / Ambiente','Forma dependente de clima ou condição ambiental'],
    stance:['Postura / Estado','Mudança de postura ou estado do mesmo Pokémon'],
    other:['Outra Forma','Variação alternativa registrada']
  };
  return info[group]||info.other;
}
function formOverviewHTML(id,baseName,baseTypes,forms){
  const gen=generationOfId(id);
  const baseTypesText=(baseTypes||[]).map(t=>TYPE_PT[t]||t).join(' / ')||'—';
  const baseCard=`<div class="form-overview-card current"><img src="${sprite(id,false)}" alt="${cap(baseName)}"><div><span class="form-badge">Forma padrão</span><strong>${cap(baseName)}</strong><small>Geração ${gen} • ${baseTypesText}</small></div></div>`;
  const altCards=(forms||[]).map(f=>{
    const [kind,how]=formMechanismInfo(f);
    const types=(f.types||[]).map(t=>TYPE_PT[t]||t).join(' / ')||'—';
    return `<button class="form-overview-card" data-special-id="${id}" data-special-open="transform" data-special-index="${f.index}"><img src="${f.art}" alt="${f.label}"><div><span class="form-badge">${kind}</span><strong>${f.label}</strong><small>${how}</small><small>Geração ${gen} • ${types}</small></div></button>`;
  }).join('');
  return `<section class="form-overview"><div class="form-overview-head"><div><span class="eyebrow">FORMAS DISPONÍVEIS</span><h3>Visão geral das formas</h3></div><span class="muted">${1+(forms?.length||0)} forma${1+(forms?.length||0)===1?'':'s'}</span></div><div class="form-overview-grid">${baseCard}${altCards}</div></section>`;
}
async function goToTransformationEntry(id,index=0,formName=''){
  ...
  requestAnimationFrame(()=>{
    let card=document.querySelector(`#dexResults .poke-card[data-id="${id}"][data-special-category="transform"][data-special-index="${index}"]`);
    if(!card&&formName){
      card=[...document.querySelectorAll(`#dexResults .poke-card[data-id="${id}"][data-special-category="transform"]`)]
        .find(el=>el.querySelector('h3')?.textContent?.toLowerCase().includes(formName.toLowerCase().replace(/-/g,' ')));
    }
    if(!card)card=document.querySelector(`#dexResults .poke-card[data-id="${id}"][data-special-category="transform"]`);
    if(card){
...
const ownTransforms=(await getTransformationForms(id)).map((f,i)=>({...f,index:i}));
const formOverview=formOverviewHTML(id,s?.name||d.name,d.types,ownTransforms);
let transformationSection='';
if(ownTransforms.length){
  transformationSection=`<h3>Transformações e Formas Alternativas</h3><div class="special-forms-panel"><div class="special-form-group"><div class="special-form-grid">${ownTransforms.map((f,i)=>`<button class="special-form-card" data-special-id="${id}" data-special-open="transform" data-special-index="${i}"><img loading="lazy" data-special-img="1" data-normal-src="${f.art}" data-shiny-src="${f.shinyArt||f.art}" src="${initialShiny&&(f.shinyArt||f.art)?(f.shinyArt||f.art):f.art}" alt="${f.label||specialFormLabel(f.formName,s?.name||d.name)}"><span><strong>${f.label||specialFormLabel(f.formName,s?.name||d.name)}</strong><small>${f.types.map(t=>TYPE_PT[t]||t).join(' / ')}</small></span></button>`).join('')}</div></div></div>`;
}
const forms=d.forms?.map(cap).join(', ')||'Forma padrão';const modalTypes=special?.types?.length?special.types:d.types;const modalName=special?(special.label||specialFormLabel(special.formName,s?.name||d.name)):cap(d.name);const modalImg=special?(initialShiny&&special.shinyArt?special.shinyArt:special.art):sprite(id,initialShiny);$('#modalContent').innerHTML=`<div class="detail-top"><img id="detailImg" src="${modalImg}"><div><span class="num">#${pad(id)} • ${regionOf(id)}</span><h2>${modalName}</h2><div class="types">${modalTypes.map(t=>`<span class="type ${t}">${TYPE_PT[t]||t}</span>`).join('')}</div><p class="muted">${special?'Forma especial selecionada':'Formas registradas: '+forms}</p><div class="detail-actions"><button id="shinyBtn">✨ Shiny: ${initialShiny?'ON':'OFF'}</button><button id="modalFav" class="fav ${favs.has(id)?'on':''}">★ Favoritar</button><button id="modalAdd" class="primary">＋ Adicionar ao time</button>${specialCategory==='transform'?'<button id="goToTransform" class="transform-jump">↗ Ir para Transformações</button>':''}</div></div></div>${formOverview}<h3>Linha Evolutiva</h3>${evo}${transformationSection}${specialSection}`;modal.classList.remove('charmander-rgb');modal.classList.add('pokemon-rgb');const typePalette={normal:['#a8a77a','#e5e2c7','#7f8064'],fire:['#f0442e','#ff9d32','#ffd35a'],water:['#2589e8','#63b9ff','#195fc1'],electric:['#f2c91c','#fff36a','#d99b00'],grass:['#35a853','#7bdc75','#1f7a3b'],ice:['#55cfd0','#b8f4f1','#3b9da8'],fighting:['#c53b32','#f06a4f','#7e201f'],poison:['#9b4bb0','#dc78e5','#682a7d'],ground:['#c9934d','#e5c57b','#8a5c2d'],flying:['#7aa6e8','#b9d2ff','#6f69b5'],psychic:['#ed4f83','#ff94b6','#a92f66'],bug:['#91a51c','#c8d94a','#596d13'],rock:['#b69b45','#ddc979','#766426'],ghost:['#6a57a5','#9d8bd4','#3e326c'],dragon:['#6245e8','#9a7cff','#3327a0'],dark:['#5b4a42','#9a8073','#2f2724'],steel:['#8fa5b5','#d1dce4','#607583'],fairy:['#e978a7','#ffb6d2','#ad4777']};const ts=special?.types?.length?special.types:(d.types||[]);const p1=typePalette[ts[0]]||['#36a8ff','#8ad8ff','#1d69c7'];const p2=typePalette[ts[1]]||p1;modal.style.setProperty('--poke-c1',p1[0]);modal.style.setProperty('--poke-c2',p1[1]);modal.style.setProperty('--poke-c3',p2[0]);modal.style.setProperty('--poke-c4',p2[1]);modal.style.setProperty('--poke-glow',p1[0]);modal.classList.remove('hidden');let shiny=!!initialShiny;$('#shinyBtn').onclick=()=>{shiny=!shiny;$('#detailImg').src=special?(shiny&&special.shinyArt?special.shinyArt:special.art):sprite(id,shiny);document.querySelectorAll('[data-evo-img]').forEach(img=>img.src=sprite(+img.dataset.evoImg,shiny));document.querySelectorAll('[data-special-img]').forEach(img=>{img.src=shiny?(img.dataset.shinySrc||img.dataset.normalSrc):img.dataset.normalSrc});$('#shinyBtn').textContent=`✨ Shiny: ${shiny?'ON':'OFF'}`};$('#modalFav').onclick=()=>{toggleFav(id,shiny);$('#modalFav').classList.toggle('on',favs.has(id));$('#modalFav').textContent=favs.has(id)?'★ Favoritado':'★ Favoritar'};$('#modalAdd').onclick=()=>addToTeam(id,shiny);const goTransform=$('#goToTransform');if(goTransform)goTransform.onclick=()=>goToTransformationEntry(id,specialIndex,special?.formName||'');document.querySelectorAll('[data-special-open]').forEach(b=>b.onclick=()=>openPokemon(Number(b.dataset.specialId||id),shiny,b.dataset.specialOpen,Number(b.dataset.specialIndex||0)));qsa('[data-evo]').forEach(b=>b.onclick=()=>openPokemon(+b.dataset.evo,shiny))}
...
function renderTeams(){return preserveViewportDuring(()=>renderTeamsCore())}
function renderTeamsCore(){
  ...
  qsa('[data-team]').forEach(b=>b.onclick=()=>{state.activeTeam=+b.dataset.team;renderTeams()});
  ...
  qsa('[data-remove-member]').forEach(b=>b.onclick=()=>{const i=+b.dataset.removeMember;t.members.splice(i,1);t.memberShiny?.splice(i,1);t.memberIV100?.splice(i,1);save();renderTeams()});
  $('#renameTeam').onclick=()=>{
    ...
    $('#confirmTeamCreate').onclick=submit;$('#cancelTeamCreate').onclick=closeTeamChooser;input.focus({preventScroll:true});input.select();input.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();submit()}};
  };
  ...
  qsa('[data-rec-add]').forEach(b=>b.onclick=()=>openTeamChooser(+b.dataset.recAdd,false));
  ensureTeamDetails(t);
}
...
function bindTeamNameForm(onCreated){
  ...
  const submit=()=>{
    ...
    if(!name){toast('Digite um nome para a equipe.');input?.focus({preventScroll:true});return}
...
function createTeam(){
  const modal=$('#teamChooser'),content=$('#teamChooserContent');
  if(document.body.dataset.scrollLocked!=='1'){document.body.dataset.teamScrollLock='1';lockPageScroll()}
  if(!modal||!content){if(document.body.dataset.teamScrollLock==='1'){delete document.body.dataset.teamScrollLock;unlockPageScroll()}toast('Não foi possível abrir o criador de equipe.');return null}
...
function openTeamChooser(id,shiny=false,iv100=false){
  const modal=$('#teamChooser'),content=$('#teamChooserContent');
  if(document.body.dataset.scrollLocked!=='1'){document.body.dataset.teamScrollLock='1';lockPageScroll()}
  if(!modal||!content){if(document.body.dataset.teamScrollLock==='1'){delete document.body.dataset.teamScrollLock;unlockPageScroll()}const fallback=teams.find(t=>t.members.length<6);if(fallback)return commitAddToTeam(fallback,id,shiny,iv100);toast('Não foi possível abrir o seletor de equipes.');return false}
...
function renderPicker(){
  ...
  preserveViewportDuring(()=>{$('#roulettePicker').innerHTML=grouped.length?grouped.map(([region,items])=>{
