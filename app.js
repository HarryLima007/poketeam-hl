...
async function getSpecialForms(id,category){
  ...
  for(const formName of specialFormCandidates(name,category)){
    try{
      ...
      if(art)forms.push({formName,art,shinyArt,types:(p.types||[]).sort((a,b)=>a.slot-b.slot).map(x=>x.type.name)});
